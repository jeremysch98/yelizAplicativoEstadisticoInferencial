...
public class CalcularConVarm extends IngresarDatos implements MostrarPro, MostrarIntConf{
    private double varm;

    public CalcularConVarm(double varm, String descripcion, int tm, double pc, double chiuno, double chidos) {
    ...
    public double CalcularLimUno(){
        if(getChiuno()>getChidos()){
            return ((getTm()-1)*this.varm)/getChiuno();
        }else{
            return ((getTm()-1)*this.varm)/getChidos();
        }  
    }
    ...
    public String MostrarPro(){
        return "- DATOS DEL PROBLEMA:"+"\n\n"+
               ...
               "    La varianza muestral (S²): "+this.varm+"\n\n"+
               "\n"+
               "- OBTENEMOS:"+"\n\n"+
               ...
               "        X²(1-α/2;v)         X²(α/2;v)"+"\n\n"+
               "    Por lo tanto:"+"\n\n"+
               "        "+gl_LimiteUnoDos()+"*"+this.varm+"              "+gl_LimiteUnoDos()+"*"+this.varm+"\n"+
               "        ---------------  < σ² <  ---------------"+"\n"+
               "            "+getChiuno()+"                      "+getChidos()+"\n\n"+
               "    Operando obtenemos:"+"\n\n"+
